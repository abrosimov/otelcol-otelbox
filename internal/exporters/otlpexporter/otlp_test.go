...
func TestProcessErrorAuthenticationRetry(t *testing.T) {
	interval := 2 * time.Hour
	for _, code := range []codes.Code{codes.Unauthenticated, codes.PermissionDenied} {
		t.Run(code.String(), func(t *testing.T) {
			original := status.Error(code, "credential rejected")
			err := processError(original, AuthRetryConfig{Enabled: true, Interval: interval})

			require.EqualError(t, err, exporterhelper.NewThrottleRetry(original, interval).Error())
			assert.False(t, consumererror.IsPermanent(err))
		})
	}
}

func TestProcessErrorAuthenticationRetryDisabled(t *testing.T) {
	err := processError(status.Error(codes.Unauthenticated, "credential rejected"), AuthRetryConfig{})
