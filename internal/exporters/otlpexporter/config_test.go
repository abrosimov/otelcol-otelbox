...
func TestAuthRetryValidation(t *testing.T) {
	for _, tt := range []struct {
		name    string
		retry   configretry.BackOffConfig
		auth    AuthRetryConfig
		wantErr string
	}{
		{
			name:    "retry disabled",
			auth:    AuthRetryConfig{Enabled: true, Interval: time.Hour},
			wantErr: `requires "retry_on_failure.enabled"`,
		},
		{
			name:    "interval missing",
			retry:   configretry.NewDefaultBackOffConfig(),
			auth:    AuthRetryConfig{Enabled: true},
			wantErr: `"retry_on_auth_failure.interval" must be positive`,
		},
		{
			name:  "enabled",
			retry: configretry.NewDefaultBackOffConfig(),
		...
		t.Run(tt.name, func(t *testing.T) {
			cfg := &Config{
				RetryConfig:  tt.retry,
				AuthRetry:    tt.auth,
				ClientConfig: configgrpc.ClientConfig{Endpoint: "localhost:4317"},
			}
			err := cfg.Validate()
			if tt.wantErr == "" {
