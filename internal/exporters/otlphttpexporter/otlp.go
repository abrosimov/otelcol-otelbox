...
func (e *baseExporter) export(ctx context.Context, requestURL string, request []byte, partialSuccessHandler partialSuccessHandler) error {
	...
	if e.config.AuthRetry.Enabled && isAuthenticationFailureStatusCode(resp.StatusCode) {
		return exporterhelper.NewThrottleRetry(formattedErr, e.config.AuthRetry.Interval)
	}
...
func isAuthenticationFailureStatusCode(code int) bool {
